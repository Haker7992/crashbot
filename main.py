...
async def on_ready():
    ...
    async def slash_analyze(interaction: discord.Interaction, name: str, action: str = "save"):
        ...
        guild_analysis[name.lower()] = {
            ...
            "roles": roles,
            "saved_at": datetime.utcnow().strftime("%d.%m.%Y %H:%M")
        }
        save_analyses()
        ...
        for cat in categories[:8]:
            ch_names = " · ".join(f"#{c['name']}" for c in cat["channels"][:4])
    ...
    @app_commands.describe(name="Название анализа (например: kanto)")
    async def slash_load(interaction: discord.Interaction, name: str):
        if interaction.user.id != config.OWNER_ID:
            await interaction.response.send_message("❌ Только для овнера.", ephemeral=True)
            return

        analysis = guild_analysis.get(name.lower())
        if not analysis:
            saved = ", ".join(f"`{k}`" for k in guild_analysis.keys()) or "пусто"
            await interaction.response.send_message(
                f"❌ Анализ `{name}` не найден.\n**Сохранённые:** {saved}",
                ephemeral=True
            )
            return
        ...
        # 1. Создаём роли
        existing_roles = {r.name for r in target.roles}
        for role_data in reversed(analysis["roles"]):
            if role_data["name"] in existing_roles:
                continue
            try:
                color_str = str(role_data.get("color", "0x000000")).replace("#", "").replace("0x", "")
                try:
        ...
        # 2. Создаём категории и каналы
        for cat_data in analysis["categories"]:
            cat = discord.utils.find(lambda c: c.name.lower() == cat_data["name"].lower(), target.categories)
            if not cat:
                try:
            ...
            existing_ch = {ch.name.lower() for ch in cat.channels}
            for ch_data in cat_data["channels"]:
                if ch_data["name"].lower() in existing_ch:
                    continue
                try:
                    ...
                    results.append(f"✅ #{ch_data['name']}")
                except Exception as e:
                    results.append(f"❌ #{ch_data['name']}: {e}")

        # 3. Каналы без категории
        existing_ch_names = {ch.name.lower() for ch in target.channels}
        for ch_data in analysis.get("no_category_channels", []):
            if ch_data["name"].lower() in existing_ch_names:
                continue
            try:
                if "voice" in ch_data["type"]:
                    await target.create_voice_channel(ch_data["name"])
                else:
                    await target.create_text_channel(ch_data["name"])
                results.append(f"✅ #{ch_data['name']}")
            except Exception as e:
                results.append(f"❌ #{ch_data['name']}: {e}")
        ...
        embed = discord.Embed(
            title=f"✅ Структура `{name}` создана",
            description=f"Источник: **{analysis['guild_name']}**\n\n✅ Создано: **{created}**\n❌ Ошибок: **{failed}**",
            color=0x00ff00
        )
        if results:
            chunk = "\n".join(results[:20])
            ...
            embed.add_field(name="Детали", value=chunk, inline=False)
        embed.set_footer(text="☠️ Kanero")
        await interaction.followup.send(embed=embed, ephemeral=True)
